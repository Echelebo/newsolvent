    ...
    if($hh->status==0){
        ...
        if($hh->activation_verify == 1 && $hh->imf_verify == 0){
            ...
            Image::make($image)->save($location);
                ...
                if(count($count)>0){
                $contentx="Acct: $userx->acct_no, DR Acct: $user->acct_no, Sent From: $user->name, Bank: Solvent Bank, Date: Carbon::now(), CR Amt: number_format($hh->amount,2), Bal: number_format($userx->balance,2), Ref: $hh->reference.";
                }
                if($set->sms_notify==1){
        ...
        if($hh->activation_verify == 0 && $hh->imf_verify == 1){
            ...
            if($user->pin==$request->activation_code){
                ...
                $b=$user->balance-$a;
                        ...
                        $content="Acct: $user->acct_no, BNF Acct: $hh->sender_acctno, Bank: $hh->sender_bankname, Date: Carbon::now(), DR Amt: number_format($hh->amount,2), Bal: number_format($user->balance,2), Ref: $hh->reference.";
                if(count($count)>0){
                $contentx="Acct: $userx->acct_no, DR Acct: $user->acct_no, Sent From: $user->name, Bank: Solvent Groups, Date: Carbon::now(), CR Amt: number_format($hh->amount,2), Bal: number_format($userx->balance,2), Ref: $hh->reference.";
                }
                if($set->sms_notify==1){
                        send_sms($user->phone, 'Solvent Groups, ' . $content);
                        if(count($count)>0){
